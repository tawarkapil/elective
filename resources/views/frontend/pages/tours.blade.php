...
@section('title')
<title>Itinerary - {{ ViewsHelper::getConfigKeyData('website_title') }}</title>
@stop
@section('content')
                      ...
                      @foreach($tours as $row)
                          ...
                          <div class="d-flex flex-wrap mb-30 tours_blog">
                            ...
                            <div class="entry-content border-1px p-20">
                              <h3 class="entry-title mt-0 pt-0"><a href="{{ $row->getDetailsPageUrl() }}">{{ $row->title }} - {{ $row->getdestination->title }}({{ $row->getdestination->getcountry->name }}) </a></h3>
