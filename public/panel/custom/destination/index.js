$(function(){
    ...
    function draw_data() {
        ...
        ajaxTbl = $('#custom-ajax-tbl').DataTable({
            ...
            responsive: true, 
            ajax: {
                url: ADMIN_HTTP_PATH + "destinations/ajax_list",
                type: "POST",
                data: {
            ...
            columns: [
                ...
                { data: 'programs', "orderable": false},
                { data: 'countries.name', "orderable": true},
    ...
    $("body").on("click", ".editBtn", function (e) {
        ...
        $('#submitFrmMdl').find("#page_headline").html('Edit Destination');
        getfillform(_this);
    });
    ...
    function getfillform(_this){
        $.ajax({
            type: "POST",
            url: ADMIN_HTTP_PATH + 'destinations/get_content',
            ...
            success: function(data){
                if (data.status == 1){
                    ...
                    $("#programs").select2().val(data.programs).trigger("change");
    ...
    $("body").on('submit', '#submitFrm', function (e) {
        ...
        $.ajax({
         type: "POST",
         url: ADMIN_HTTP_PATH + 'destinations/addnewajax',
         data: dataS,
        contentType: false,
         cache: false,
         processData:false,
    ...
    $('body').on('click', '.delConfirmationBtn', function(e){
        ...
        $.ajax({
         type: "POST",
         url: ADMIN_HTTP_PATH + 'destinations/delete',
         data: 'id=' + id + "&_token=" + CSRF_TOKEN,
         dataType: 'json',
         success: function(data){
            error_remove();
    ...
    $("body").on("click", ".confirm_status", function (e) {
         ...
         $.ajax({
             type: "POST",
                 url: ADMIN_HTTP_PATH + "destinations/update_status",
    ...
    $('#uploadFiles').fileupload({
        ...
        url : ADMIN_HTTP_PATH + "destinations/uploadChunkFile",
        autoUpload: true,
        add: function (e, data) {
            var goUpload = true;
