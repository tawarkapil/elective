$(function(){
     ...
     CKEDITOR.replace('common_condition');
     CKEDITOR.replace('program_focus_point');
    ...
    function draw_data() {
        ...
        ajaxTbl = $('#custom-ajax-tbl').DataTable({
            ...
            responsive: true, 
            ajax: {
                url: ADMIN_HTTP_PATH + "programs/ajax_list",
                type: "POST",
                data: {
    ...
    var id = '';
     ...
     $("body").on("click", '.addNewBtn', function (e) {
        ...
        CKEDITOR.instances.common_condition.setData("", function () {});
        CKEDITOR.instances.program_focus_point.setData("", function () {});
        $('#submitFrmMdl').find('#page_headline').text('Add Program');
    ...
    $("body").on("click", ".editBtn", function (e) {
        ...
        $('#submitFrmMdl').find("#page_headline").html('Edit Program');
        getfillform(_this);
    });
    ...
    function getfillform(_this){
        $.ajax({
            type: "POST",
            url: ADMIN_HTTP_PATH + 'programs/get_content',
            data: 'id=' + id + "&_token=" + CSRF_TOKEN,
            dataType: 'json',
            success: function(data){
                if (data.status == 1){
                    CKEDITOR.instances.description.setData(data.description, function () {});
                    CKEDITOR.instances.common_condition.setData(data.common_condition, function () {});
                    CKEDITOR.instances.program_focus_point.setData(data.program_focus_point, function () {});
                    ...
                    $("#common_condition").val(data.common_condition);
                    $("#program_focus_point").val(data.program_focus_point);
    ...
    $("body").on('submit', '#submitFrm', function (e) {
        ...
        var program_focus_point = CKEDITOR.instances.program_focus_point.getData();
        var common_condition = CKEDITOR.instances.common_condition.getData();

        dataS.append('id', id);
        dataS.append('description', description);
         dataS.append('common_condition', common_condition);
        dataS.append('program_focus_point', program_focus_point);
        ...
        $.ajax({
         type: "POST",
         url: ADMIN_HTTP_PATH + 'programs/addnewajax',
         data: dataS,
         contentType: false,
         cache: false,
         processData:false,
    ...
    $('body').on('click', '.delConfirmationBtn', function(e){
        ...
        $.ajax({
         type: "POST",
         url: ADMIN_HTTP_PATH + 'programs/delete',
         data: 'id=' + id + "&_token=" + CSRF_TOKEN,
         dataType: 'json',
         success: function(data){
            error_remove();
    ...
    $("body").on("click", ".confirm_status", function (e) {
         ...
         $.ajax({
             type: "POST",
                 url: ADMIN_HTTP_PATH + "programs/update_status",
    ...
    $('#uploadFiles').fileupload({
        ...
        url : ADMIN_HTTP_PATH + "programs/uploadChunkFile",
        autoUpload: true,
        add: function (e, data) {
            var goUpload = true;
